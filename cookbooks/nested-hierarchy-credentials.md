...
Load credentials dynamically based on context (division, environment, provider, country) using hierarchical HCL blocks.

**Use case:** Managing service provider credentials across multiple divisions, environments, and region-specific configurations.
...
Use nested hierarchy blocks with `Huckle::loadEnv()` to load context-specific credentials:

```php
use Cline\Huckle\Facades\Huckle;

// Load credentials for FI division, production environment, provider_a, region EE
Huckle::loadEnv('credentials.hcl', [
    'division' => 'FI',
...
## Loading Context-Specific Credentials

### Provider-Level Only

```php
// Load provider_b credentials for FI production (no country-specific config needed)
Huckle::loadEnv('credentials.hcl', [
    'division' => 'FI',
    'environment' => 'production',
    'provider' => 'provider_b',
...
```php
// Load provider_a credentials for FI production, region EE
$exports = Huckle::loadEnv('credentials.hcl', [
    'division' => 'FI',
    'environment' => 'production',
    'provider' => 'provider_a',
    'country' => 'EE',
...
class ExternalApiService
{
    ...
    {
        // Load credentials for the target region
        Huckle::loadEnv(base_path('credentials.hcl'), [
...
$config = $parser->parseFile(base_path('credentials.hcl'));
...
- `client` - Per-client/customer credentials
...
$config = $parser->parseFile('credentials.hcl'); // with country "INVALID" block
```
...
$parser = new HuckleParser();
$config = $parser->withoutGeoValidation()->parseFile('credentials.hcl');

// Or use the explicit method:
$config = $parser->withGeoValidation(false)->parseFile('credentials.hcl');
```
...
```php
$parser = new HuckleParser();
$result = $parser->validateFile('credentials.hcl');
