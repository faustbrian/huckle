...
final class ConfigEncryptCommand extends Command
{
     ...
     *
     * Accepts a file or directory path as the primary argument, with options for
     * encryption key specification (auto-generated if omitted), cipher algorithm
     * selection, environment handling, file cleanup, and glob pattern filtering
     ...
     * prefix stripping when using the application key. The explicit --key option
     * takes precedence over --app-key or configuration defaults.
     *
    ...
    private function resolveEncryptionKey(?string $key, bool $useAppKey, ?string $cipher): string
    {
        ...
        if ($shouldUseAppKey) {
            ...
            if ($appKey === null) {
                throw MissingAppKeyException::notConfigured();
            }
