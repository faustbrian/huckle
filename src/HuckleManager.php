...
final class HuckleManager
{
    ...
    /**
     * Runtime enabled state (overrides config when set).
     */
    private bool $enabled = false;

    /**
     * Whether the enabled state has been explicitly set at runtime.
     */
    private bool $enabledOverridden = false;
     ...
     *
     * Parses the HCL file and caches the result for subsequent calls.
     *
     * @param null|string $path Optional path override to load from a specific file
     *
     ...
     *
     * Parses the file, filters nodes by context, and exports matching environment
     * variables to PHP's environment ($_ENV, $_SERVER, putenv).
     *
     * @param string                $path    Path to the HCL configuration file
     * @param array<string, string> $context Context filter (e.g., ['partition' => 'FI', 'environment' => 'production'])
     *
     * @return array<string, string> Exported environment variable key-value pairs
     */
     ...
     *
     * @return HuckleConfig The cached or newly loaded configuration
     */
     ...
     *
     * @return null|Node The node or null if not found
     */
     ...
     * Get all nodes.
     *
     * @return Collection<string, Node> All configuration nodes indexed by path
     */
     ...
     *
     * @return Collection<string, Node> Top-level partition nodes indexed by name
     */
     ...
     *
     * @param array<string, string> $context Context filter (partition, environment, provider, etc.)
     *
     ...
     *
     * @return array<string, string> Merged exports from context-matching nodes
     */
     ...
     *
     * Writes to both environment variables and Laravel Config (when mappings or
     * config blocks are defined). Ensures compatibility with config:cache where
     * env() calls are cached.
     *
     * @param array<string, string> $context Context filter
     *
     * @return self Fluent interface for method chaining
    ...
    public function exportContextToEnv(array $context): self
    {
        ...
        // Also apply mapped exports to Laravel Config
        $this->applyMappedExportsToConfig($exports);

        // Apply direct config blocks
        $configs = $this->configsForContext($context);
    ...
    /**
     * Export context-matching values directly to Laravel Config.
     *
     * Writes values to Laravel's config repository using:
     * 1. Direct config blocks from nodes (config { "path" = "value" })
     * 2. Global mappings from HCL (mappings { ENV_KEY = "config.path" })
     * 3. PHP config mappings (huckle.php mappings array)
     *
     * Designed for use with config:cache where env() is cached.
     *
     * @param array<string, string> $context Context filter
     *
     * @return self Fluent interface for method chaining
     */
    public function exportContextToConfig(array $context): self
    {
        if ($this->isDisabled()) {
            return $this;
        }

        // Apply exports via mappings
        $exports = $this->exportsForContext($context);
        $this->applyMappedExportsToConfig($exports);
    ...
    /**
     * Get config mappings for a given context.
     *
     * @param  array<string, string> $context Context filter
     * @return array<string, string> Config path to value mappings
     */
    public function configsForContext(array $context): array
    {
        return $this->config()->configsForContext($context);
    }

    /**
     * Get the global mappings (env key -> config path).
     *
     * Merges mappings from HCL mappings block with PHP config mappings.
     * HCL mappings take precedence over PHP config.
     *
     * @return array<string, string> Env var name to config path mappings
     */
    public function mappings(): array
    {
        /** @var array<string, string> $phpMappings */
        $phpMappings = Config::get('huckle.mappings', []);
        $hclMappings = $this->config()->mappings();

        // HCL mappings override PHP config mappings
        return [...$phpMappings, ...$hclMappings];
    }
    ...
    public function getConfigPath(): string
    {
        ...
        // Check explicit environment mappings first
        if (isset($environments[$environment]) && file_exists($environments[$environment])) {
            return $environments[$environment];
        }

        // Auto-discover environment-specific files if enabled
        if (Config::get('huckle.auto_discovery', false)) {
            $autoDiscoveredPath = $this->app->basePath('.huckle.'.$environment);

            if (file_exists($autoDiscoveredPath)) {
                return $autoDiscoveredPath;
            }
        }
    ...
    /**
     * Check if Huckle is enabled.
     *
     * @return bool True if Huckle is enabled
     */
    public function isEnabled(): bool
    {
        if ($this->enabledOverridden) {
            return $this->enabled;
        }

        /** @var bool */
        return Config::get('huckle.enabled', true);
    }

    /**
     * Check if Huckle is disabled.
     *
     * @return bool True if Huckle is disabled
     */
    ...
    /**
     * Apply exports to Laravel Config using global mappings.
     *
     * Looks up each export key in the combined mappings (HCL + PHP config)
     * and writes the value to the corresponding config path.
     *
     * @param array<string, string> $exports Env key to value pairs
     */
    private function applyMappedExportsToConfig(array $exports): void
    {
        $mappings = $this->mappings();

        foreach ($exports as $envKey => $value) {
            if (!isset($mappings[$envKey])) {
                continue;
            }

            Config::set($mappings[$envKey], $value);
        }
    }
