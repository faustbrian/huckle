...
describe('HuckleManager', function (): void {
    ...
    describe('enable/disable', function (): void {
        test('isEnabled returns true by default from config', function (): void {
            Config::set('huckle.enabled', true);

            expect($this->manager->isEnabled())->toBeTrue();
            expect($this->manager->isDisabled())->toBeFalse();
        });

        test('isEnabled returns false when config is false', function (): void {
            Config::set('huckle.enabled', false);

            expect($this->manager->isEnabled())->toBeFalse();
            expect($this->manager->isDisabled())->toBeTrue();
        });

        test('enable() overrides config to true', function (): void {
            Config::set('huckle.enabled', false);
        ...
        test('disable() overrides config to false', function (): void {
            Config::set('huckle.enabled', true);
        ...
        test('enable() and disable() return self for chaining', function (): void {
            expect($this->manager->enable())->toBe($this->manager);
            expect($this->manager->disable())->toBe($this->manager);
        });

        test('exportToEnv does nothing when disabled', function (): void {
            $this->manager->disable();
        ...
        test('exportAllToEnv does nothing when disabled', function (): void {
            $this->manager->disable();
        ...
        test('exportContextToEnv does nothing when disabled', function (): void {
            $this->manager->disable();

            putenv('DB_HOST');
            unset($_ENV['DB_HOST'], $_SERVER['DB_HOST']);

            $this->manager->exportContextToEnv(['environment' => 'production']);

            expect(getenv('DB_HOST'))->toBeFalse();
        });

        test('exportContextToConfig does nothing when disabled', function (): void {
            $this->manager->disable();

            Config::set('database.connections.pgsql.host');

            $this->manager->exportContextToConfig(['environment' => 'production']);

            expect(Config::get('database.connections.pgsql.host'))->toBeNull();
        });
    });
